...
private:
	...
	std::complex<double> tp_wave;	// Fresnel terms for the P-polarized wave 
	
public:

	double Reflectance(double, double, double, double, double, double, double, double, double);
	double Transmissivity(double, double, double, double, double, double, double, double, double);
...
double SingleLayerSPR::Transmissivity(double theta_i, double real_1, double imag_1, double real_2, double imag_2, double n_prism, double wavelength, double d_1, double d_2){
	...
	std::complex<double> n_metal_2(real_2, imag_2); 
	std::complex<double> i(0, 1); // Defining an imaginary number
	std::complex<double> r(2, 0); // Defining a real number
	...
	// ****
	// Calculing Fresnel transmission coefficientes for the P_polarized wave between interface
	// ***

	std::complex<double> num (r*q_1);
	std::complex<double> den ((M_11 + M_12*q_2)*q_1 + (M_21 + M_22*q_2));

	std::complex<double> tp_wave (num / den);

	return (abs(tp_wave));  // Return transmissivity value for the two-layer system
} 